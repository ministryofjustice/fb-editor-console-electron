...
async function checkGIT () {
  ...
  installationLogger.log('Detecting platform: ' + platformName)
  ...
  if (platformName === 'mac') {
    try {
      ...
      installationLogger.error(message)
      installationLogger.error('Git is not installed. Installing Git')
      try {
        await app.displayNotification(
          'Git is not installed. Installing Xcode tools ...', { phase: 'Install Editor' }
        )
        execSync(`
          xcode-select --install
        `)
        // Wait for xcode installation to finish.
        await sleep(120000)
        // Notifying the user
        await app.displayNotification(
          'Wait for Xcode installation', { phase: 'Install Editor' }
        )
        // Wait for xcode installation to finish.
        await sleep(300000)
        app.relaunch(5)
