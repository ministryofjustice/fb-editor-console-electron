...
async function installDependencies () {
  installationLogger.log('Verifying if GIT is installed')
  await checkGIT()
...
async function checkGIT () {
  try {
    ...
    installationLogger.log('Git is not installed.')
    await app.displayNotification(
      'Before you can install Form Builder you will need to install Git. Installation instructions: https://bit.ly/3ggfhbF', { phase: 'Install Editor' }
    )
    await sleep(20000)
    app.exit(0)
  }
...
function systemSync (cmd) {
  child_process.exec(cmd, (err, stdout, stderr) => {
    installationLogger.log('Running command: ' + cmd)
    installationLogger.log('stdout is:' + stdout)
    installationLogger.log('stderr is:' + stderr)
    installationLogger.log('error is:' + err)
  }).on('exit', code => installationLogger.log('final exit code is', code))
}
...
function installEditorDependencies () {
  ...
  if (platformName === 'win' ) {
    ...
    `)
    systemSync(`
      ${app.paths.nvs}\nvs.cmd use 12.4.0 && cd "${app.paths.editor}" && npm install
    `)
  } else {
    try {
      execSync(`
